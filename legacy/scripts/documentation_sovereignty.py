...
try:
    from backend.app.services.traceability_service import \
        TraceabilityService as RealTraceabilityService
except ImportError:
    ...
    class PlaceholderTraceabilityService:
        ...
        def record_data_creation(self, **kwargs):
            pass

        def record_data_access(self, **kwargs):
            pass
...
try:
    from backend.app.services.documentation_sovereignty import \
        DocumentationSovereigntyService as RealDocSovereigntyService
except ImportError:
    print(
        "Warning: Real DocumentationSovereigntyService not found "
        "in backend.app.services, using local definition."
    )
...
class LocalDocumentationSovereigntyService:
    ...
    def __init__(self, db=None, docs_dir=None, integrity_file=None):
        ...
        self.integrity_file = (
            ...
            else (self.docs_dir / "INTEGRITY.json")
        )
        self.traceability: TraceabilityServiceType = TraceabilityServiceType(db)

        if not self.docs_dir.exists():
            print("Warning: Documentation directory not found: " f"{self.docs_dir}")
    ...
    def generate_integrity_map(self):
        ...
        for doc_file in self.docs_dir.rglob("*.md"):
            ...
            if git:
                try:
                    ...
                    commits = list(
                        repo.iter_commits(paths=repo_relative_path, max_count=1)
                    )
                    if commits:
                        commit = commits[0]
                        last_commit = {
        ...
        try:
            ...
            print("Error writing integrity map to " f"{self.integrity_file}: {e}")
            return None
    ...
    def verify_integrity(self):
        ...
        if not self.integrity_file.exists():
            return {
                ...
                "message": ("Integrity map not found: " f"{self.integrity_file}"),
            }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": (
                    "Error reading integrity map " f"{self.integrity_file}: {e}"
                ),
            }
                ...
                results["violations"].append(
                    {
                        ...
                        "type": "missing",
                        "details": (
                            "Document listed in integrity map no " "longer exists"
                        ),
                    }
                )
                continue
...
DocumentationSovereigntyService = (
    RealDocSovereigntyService
    if RealDocSovereigntyService
    else LocalDocumentationSovereigntyService
)
...
def cli_main():
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate integrity map"
    )
    parser.add_argument(
        "--verify",
        action="store_true",
    ...
    parser.add_argument(
        "--fix",
        action="store_true",
        help=("Regenerate integrity map to fix discrepancies " "(use with caution)"),
    )
    parser.add_argument(
        "--docs-dir",
        help=(
            "Path to docs directory (defaults to ./docs relative to "
            ""
            "project root)"
            ""
        ),
    ...
    parser.add_argument(
        "--integrity-file",
        help=("Path to integrity JSON file (defaults to './docs/INTEGRITY.json')"),
    )

    args = parser.parse_args()

    docs_dir_path = (
        Path(args.docs_dir).resolve() if args.docs_dir else (project_root / "docs")
    )
    integrity_file_path = (
        Path(args.integrity_file).resolve()
        if args.integrity_file
        else (docs_dir_path / "INTEGRITY.json")
    )

    service = DocumentationSovereigntyService(
        docs_dir=docs_dir_path, integrity_file=integrity_file_path
    )

    if args.generate or args.fix:
        action = "Generating" if args.generate else "Regenerating (fixing)"
        print(f"{action} documentation integrity map for " f"'{service.docs_dir}'...")
        result = service.generate_integrity_map()
        if result:
            doc_count = len(result.get("documents", {}))
            action_verb = "generated" if args.generate else "regenerated"
            print(
                f"✅ Integrity map {action_verb} at "
                f"'{service.integrity_file}' with {doc_count} documents"
            )
        else:
            action_verb = "generate" if args.generate else "regenerate"
            print(f"❌ Failed to {action_verb} integrity map.")
            sys.exit(1)

    elif args.verify:
        print(
            "Verifying documentation integrity using " f"'{service.integrity_file}'..."
        )
        ...
        if result["status"] == "verified":
            print(
                f"✅ All {checked_count} documentation file(s) verified - "
                "sovereignty intact"
            )
            ...
            for violation in result.get("violations", []):
                if isinstance(violation, dict):
                    ...
                    print(f"  - {file_path}: {v_type} - {details}")
                else:
            ...
            if result.get("new_files"):
                ...
                for new_file in result["new_files"]:
                    print(f"  - {new_file}")
            print(
                "\nRun with --fix to accept current state and "
                "regenerate the integrity map."
            )
            sys.exit(1)
            ...
            print(
                f"🟡 Integrity map '{service.integrity_file}' exists "
                "but contains no documents."
            )
            print(
                "Run with --generate to populate it if documentation "
                ""
                "files exist."
                ""
            )
