...
def get_directories_needing_readme():
    ...
    for root, dirs, files in os.walk('.'):
        if any(exclude in root for exclude in ['.git', 'node_modules', '__pycache__', '.next']):
            continue
        ...
        if 'README.md' not in files and (
            ...
            '/docs/' in root or 
            '/tests/' in root or
            len([f for f in files if not f.startswith('.')]) > 0
        ):
    ...
    if os.path.exists(readme_template):
        for dir_path in get_directories_needing_readme():
            ...
            if not os.path.exists(dest_path):
                ...
                copy_template(readme_template, dest_path, {
